...
screen* InitializeSDL( int width, int height, bool fullscreen, int screen_width, int screen_height);
...
void SDL_Renderframe(screen* s)
{
  ...
  DestR.x = 0;
  DestR.y = 0;
  DestR.w = s->screen_width;
  DestR.h = s->screen_height;

  SDL_UpdateTexture(s->texture, NULL, s->buffer, s->width*sizeof(uint32_t));
  SDL_RenderClear(s->renderer);
  SDL_RenderCopy(s->renderer, s->texture, &SrcR, &DestR);
